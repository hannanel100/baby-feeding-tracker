...
import { Baby, Feeding, FeedingInput } from "@/types";

interface FeedingManagerProps {
  babies: Baby[];
  feedings: Feeding[];
  addFeeding: (feeding: Omit<Feeding, "id">) => void;
  updateFeeding: (feeding: Feeding) => void;
...
export function FeedingManager({
  ...
  const [newFeeding, setNewFeeding] = useState<FeedingInput>({
    babyId: "1",
    type: "bottle",
    amount: "",
    duration: "",
  ...
  const handleAddFeeding = () => {
    if (newFeeding.babyId && newFeeding.type && newFeeding.time) {
      addFeeding({
        babyId: Number(newFeeding.babyId),
        type: newFeeding.type,
        amount:
          newFeeding.type === "bottle"
            ? parseFloat(newFeeding.amount)
            : undefined,
        duration:
          newFeeding.type === "breast"
            ? parseInt(newFeeding.duration)
            : undefined,
      ...
      <CardContent>
        <div className="flex flex-wrap gap-4 mb-4">
          ...
          {newFeeding.type === "bottle" && (
            <Input
              ...
              onChange={(e) =>
                setNewFeeding({ ...newFeeding, amount: e.target.value })
              }
          ...
          {newFeeding.type === "breast" && (
            <Input
              ...
              onChange={(e) =>
                setNewFeeding({ ...newFeeding, duration: e.target.value })
              }
              className="w-[150px] bg-white"
            />
          )}
          <Input
            ...
            onChange={(e) =>
              setNewFeeding({ ...newFeeding, time: e.target.value })
            }
        ...
        <div className="space-y-4">
          {feedings.map((feeding) => (
            <div
              key={feeding.id}
              className="flex flex-wrap items-center gap-2 p-2 bg-white rounded-lg shadow"
            >
              <Select
                value={feeding.babyId.toString()}
                onValueChange={(value) =>
                  updateFeeding({ ...feeding, babyId: Number(value) })
                }
              >
